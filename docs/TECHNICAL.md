...
- 将同一账户、同一自然月的真实买入合并为定投批次，在 K 线中比较冻结的历史情景区间与实际含费净收益；
- 将确定性行情、成本和交易统计交给兼容 OpenAI Chat Completions 的模型做结构化复盘；
- 在本机 SQLite 中持久化所有业务数据。

系统不提供实盘下单、券商账户同步、实时 WebSocket 行情或投资建议。**这是永久架构边界：项目只负责记录、计算、回测、预测和复盘，任何模块都不得创建、提交或执行订单，也不得增加自动或半自动下单能力。**
...
| `manual_levels` | 手工支撑、阻力、观察位的当前状态与有效区间 |
| `manual_level_versions` | 每次创建、修改、归档和恢复形成的不可变价格位版本 |
| `trade_analysis_snapshots` | 交易发生时的历史背景 |
| `trade_outcomes` | 交易发生后的验证结果 |
| `dca_forecast_runs` | 月度买入批次在指定算法、行情口径和数据截止日下的冻结预测快照 |
| `dca_forecast_points` | 预测快照第0至60个交易日的P20、P50、P80及全历史基准中位数 |
...
| 方法 | 路径 | 用途 |
| --- | --- | --- |
| GET | `/api/instruments/:id/price-levels` | 获取当前价格位、版本、有效区间及验证统计 |
| POST | `/api/instruments/:id/price-levels` | 新增价格位并生成首个版本 |
| PATCH | `/api/price-levels/:id` | 修改价格位形成新版本，或使用 `RESTORE` 恢复归档项 |
| DELETE | `/api/price-levels/:id` | 归档价格位；不物理删除历史 |

价格位生效日取服务端活动行情系列的最新日期，不接受客户端回填过去日期。修改时保留旧版本，新版本仅从新的生效日起绘制；归档会关闭当前有效区间，恢复则创建新的有效区间。验证统计只读取生效后的 K 线：日内高低价覆盖价格位记为触及，连续触及算一次事件；支撑位收盘跌破、阻力位收盘突破、观察位收盘穿越记为首次破位。5/20/60日结果是首次触及后的价格表现，不含交易手续费。

### 11.6 月度定投情景与实际收益

| 方法 | 路径 | 用途 |
| --- | --- | --- |
| GET | `/api/instruments/:id/dca-analysis` | 获取月度批次、冻结预测、实际含费曲线和校准指标 |
| POST | `/api/instruments/:id/dca-analysis` | 保留旧快照并重建当前算法版本的预测 |

同一股票、账户和自然月的 BUY 交易合并为一个批次，锚点为该月最后一笔买入日期。预测特征包括距 MA20/MA60、20/60日动量、ATR、成交量比和20/60/120/250日区间位置。历史候选样本必须在当前买入日前完整走完60个交易日；相邻不足5个交易日的候选不会重复入选，最多保留120个相似样本。

预测输出0至60个交易日的P20、P50、P80，并使用相同数量、买入费和预计卖出费模型计算历史净收益。少于20个样本时不绘制预测；20至49个为低可信度、50至99个为中等、100个及以上为较高。实际曲线将 FIFO 已卖出净现金与剩余持仓按当日收盘价估值合并，扣除真实卖出手续费和预计平仓费后再除以含买入费投入。

预测输入哈希只包含买入日前行情，因此买入日及之后的行情不会改变冻结结果。手工重建生成新快照，旧版本继续保留以支持预测校准。所有计算均在本地完成，不调用券商或下单接口。
...
- 定投预测是历史相似情景的统计区间，不是收益承诺；小于20个有效样本时只展示真实含费收益，不生成预测线。
- 手工价格位是带版本和生效日的可验证假设，不是止损单、限价单或交易信号；触及后的收益仅为价格表现。
- 永久不接入券商下单、订单路由、自动交易或半自动交易；券商属性未来若加入，也只能用于手续费、账户归类或成交数据导入。
