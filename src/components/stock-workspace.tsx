...
import { Database, HardDrive, Layers3, Plus, RefreshCw, RotateCcw, Settings2, Sparkles } from "lucide-react";
...
export function StockWorkspace({ data, twelveDataConfigured, eodhdConfigured, aiConfigured, aiModel }: { data: StockWorkspaceData; twelveDataConfigured: boolean; eodhdConfigured: boolean; aiConfigured: boolean; aiModel: string }) {
  ...
  const [dialog, setDialog] = useState<{ date: string; price: number; trade?: TradeWithAnalysis | null } | null>(null); const [settingsOpen, setSettingsOpen] = useState(false); const [marketDataOpen, setMarketDataOpen] = useState(false); const [aiAnalysisOpen, setAiAnalysisOpen] = useState(false); const [ma20, setMa20] = useState(true); const [ma60, setMa60] = useState(true); const [showTrades, setShowTrades] = useState(true); const [refreshing, setRefreshing] = useState(false); const [message, setMessage] = useState(""); const csvRef = useRef<HTMLInputElement>(null);
