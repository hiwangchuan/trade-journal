...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav">{nav.map(({ href, label, icon: Icon }) => { const active = href === "/" ? pathname === "/" : pathname.startsWith(href); return <Link key={href} href={href} className={cn("nav-item", active && "active")}><Icon size={17} strokeWidth={1.7} />{label}</Link>; })}</nav>
    ...
    <nav className="mobile-nav">{nav.map(({ href, label, icon: Icon }) => <Link key={href} href={href}><Icon size={17} />{label}</Link>)}</nav>
  </div>;
}
