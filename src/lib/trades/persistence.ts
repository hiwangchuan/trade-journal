...
import { activateMarketSeries, assessMarketSeriesQuality, getCandlesForActiveSeries, getOrCreateConfiguredSeries, mergeCandles } from "@/lib/market-data/storage";
...
export function recalculateTrade(tradeId: number) {
  ...
  const candles = getCandlesForActiveSeries(trade.instrumentId);
...
export function recalculateTradesForInstrument(instrumentId: number) {
...
function mergeCandleDataset(instrumentId: number, candles: Candle[], adjustment: PriceAdjustment) {
  const series = getOrCreateConfiguredSeries(instrumentId);
  if (series.adjustment !== adjustment) throw new Error("K线复权方式与股票配置不一致。");
  const result = mergeCandles(series, candles);
  activateMarketSeries(series.id);
  assessMarketSeriesQuality(series.id);
...
export function upsertCandles(instrumentId: number, candles: Candle[], adjustment: PriceAdjustment = candles[0]?.adjustment ?? "raw") {
  return mergeCandleDataset(instrumentId, candles, adjustment);
}

export function replaceCandlesAndRecalculate(instrumentId: number, candles: Candle[], adjustment: PriceAdjustment) {
  const result = mergeCandleDataset(instrumentId, candles, adjustment);
  if (result.inserted || result.updated) recalculateTradesForInstrument(instrumentId);
  return result;
}
